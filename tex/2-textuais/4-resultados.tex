...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccccc}
        \toprule
        \textbf{Modalidade} & \textbf{Var. Redução (\%)} & \textbf{Curtose (antes)} & \textbf{Curtose (depois)} & \textbf{SNR (dB)} & \textbf{Amostras Winsorizadas} \\
        \midrule
...
Identificou-se que os sinais não apresentaram lacunas significativas (gaps $> 1$s) que requeressem interpolação nos dados do sujeito 000. A função de interpolação foi executada corretamente em todos os demais sujeitos sem ocorrência de erros de indexação, após a correção do \textit{bug} na função \texttt{interpolate\_gaps()}.

\subsection{Manutenção de fNIRS}

Conforme planejado na discussão do Estágio 3, a modalidade fNIRS foi mantida no pipeline de limpeza apesar de apresentar SNR constante de -60 dB e efeito de tamanho negligible. A justificativa para manutenção é que a filtragem passa-banda (0,01--0,1 Hz) reduz a variância em mais de 80\% (Tabela \ref{tab:cleaning_summary}), sugerindo que mesmo um sinal ruidoso pode beneficiar-se do pré-processamento para análises de tendências lentas.
